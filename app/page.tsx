      ...
      try {
        ...
        if (supabase && savedUserId) {
          ...
          if (data) {
            ...
            mergedPlaces = Array.from(
              new Set([...localPlaces, ...serverPlaces]),
            );

            console.log(
              "📊 [6.2단계 - Supabase] 서버 방문지 데이터 동기화 완료:",
              serverPlaces,
            );
          }
        } else {
          console.warn(
            "⚠️ Supabase 데이터베이스 연결 불가 또는 신규 유저입니다. 로컬 데이터로 대체 가동합니다.",
          );
        }
      } catch (error: any) {
        console.error(
          "🚨 [6.2단계] 서버 데이터 로드 실패 (온디바이스 모드 가동):",
          error.message,
        );
      } finally {
